...
Personal portfolio for **Jean "Velombe" Chrinot** — full-stack developer & AI builder. This repo is portfolio-only: it was extracted from a larger Next.js SaaS boilerplate repo (`new-version`, a sibling directory) with everything not needed to render the live site removed — no dashboard, no admin, no auth, no database, no billing, no AI assistant/workflows, no blog/docs/marketplace, no charts.
...
**Light/dark tokens:** `velombe.css`'s custom properties (`--v-bg`, `--v-surface`, `--v-line`, `--v-fg`, `--v-muted`, `--v-faint`, `--v-accent`, etc.) are split `.velombe { /* light */ }` / `.dark .velombe { /* dark */ }`, mirroring how `styles/globals.css` splits `:root`/`.dark` for the rest of Tailwind's semantic tokens. Every component references these via arbitrary-value utilities (`bg-[var(--v-surface)]`, `text-[var(--v-fg)]`) — never raw `zinc-*`/`white` Tailwind classes. `next-themes`' `defaultTheme` is `"dark"` (not `"system"`) in `components/providers/index.tsx`: the whole design is one dark composition, so a first-time visitor should land there regardless of OS preference.

**Never use hardcoded Tailwind palette classes** (`bg-zinc-900`, `text-white`) inside `components/velombe/*` — always the `--v-*` tokens above.
...
components/velombe/
  ...
  velombe.css              # Scoped stylesheet — tokens, accent budget, all custom animations/masks
...
lib/data/projects.ts       # PROJECTS — imported by components/velombe/case-studies.ts
...
1. **No hardcoded strings in `components/velombe/*` components** — all copy comes from `data.ts`/`case-studies.ts`
2. **No hardcoded Tailwind colors** — use the `--v-*` tokens in `velombe.css`, referenced via `[var(--v-*)]` arbitrary values
...
6. Keep `components/velombe/` self-contained — resist reaching for `components/ui` or similar shared infrastructure; if the folder ever needs to be forked or deleted again, that isolation is what makes it possible
